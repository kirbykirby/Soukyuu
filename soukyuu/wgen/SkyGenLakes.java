...
public class SkyGenLakes extends WorldGenerator
{

    private int blockIndex;

    public SkyGenLakes(int i)
    {
        blockIndex = i;
    }
    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
        ...
        for(int l1 = 0; l1 < 16; l1++)
        {
            for(int l2 = 0; l2 < 16; l2++)
            {
                ...
                {
                    if(aflag[(l1 * 16 + l2) * 8 + l3] && world.getBlockId(i + l1, (j + l3) - 1, k + l2) == InitBlock.skyDirt.blockID && world.getSavedLightValue(EnumSkyBlock.Sky, i + l1, j + l3, k + l2) > 0)
                    {
                        world.setBlock(i + l1, (j + l3) - 1, k + l2, InitBlock.skyGrass.blockID);
                    }
                }
            ...
            for(int i2 = 0; i2 < 16; i2++)
            {
                for(int i3 = 0; i3 < 16; i3++)
                {
                    for(int i4 = 0; i4 < 8; i4++)
                    {
                        ...
                        {
                            world.setBlockAndMetadata(i + i2, j + i4, k + i3, InitBlock.skyStone.blockID, 0);
                        }
                    }
